...
function AppRoutes() {
  ...
  const [user, setUser] = useState({ name: 'Guest', email: 'guest@example.com', savedStories: 0, avatar: '' }); // Default user state
  ...
  return (
    <Layout showNavigation={!isLoginPage}>
      ...
      <Routes>
        ...
        <Route path="/" 
               element={ user && user.email !== 'guest@example.com' ? <Navigate to="/dashboard" /> : <Login />} />
