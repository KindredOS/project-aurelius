...
export async function loginUser(userId) {
  const response = await fetch(`${getApiUrl()}/edu/users/login/${userId}`);
...
export async function deleteFleetData(userId, tileId) {
  const response = await fetch(`${getApiUrl()}/edu/users/fleet/${userId}/${tileId}`, {
    method: "DELETE",
  });
...
export async function getFleetData(userId) {
  const response = await fetch(`${getApiUrl()}/edu/users/fleet/${userId}`);
